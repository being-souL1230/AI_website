...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const updateCellOpacities = () => {
      ...
      if (hoveredSquareRef.current) targets.set(`${hoveredSquareRef.current.x},${hoveredSquareRef.current.y}`, 1);
    ...
    const updateAnimation = () => {
      ...
      switch (direction) {
        case 'right': gridOffset.current.x = (gridOffset.current.x - s + wrapX) % wrapX; break;
        case 'left': gridOffset.current.x = (gridOffset.current.x + s + wrapX) % wrapX; break;
        case 'up': gridOffset.current.y = (gridOffset.current.y + s + wrapY) % wrapY; break;
        case 'down': gridOffset.current.y = (gridOffset.current.y - s + wrapY) % wrapY; break;
        case 'diagonal': gridOffset.current.x = (gridOffset.current.x - s + wrapX) % wrapX; gridOffset.current.y = (gridOffset.current.y - s + wrapY) % wrapY; break;
      }
      updateCellOpacities();
      drawGrid();
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (isHex) {
        ...
        if (!hoveredSquareRef.current || hoveredSquareRef.current.x !== col || hoveredSquareRef.current.y !== row) {
          if (hoveredSquareRef.current && hoverTrailAmount > 0) { trailCells.current.unshift({ ...hoveredSquareRef.current }); if (trailCells.current.length > hoverTrailAmount) trailCells.current.length = hoverTrailAmount; }
